...
koefs = [6.8, 1.7, 5.7, 9.9, 8.9, 0.5, 6.1, 9.9, 0.7, 3.2, 8.4]
...
print('y_min = ',y_min)
print('y_max = ',y_max)
print('f(x1,x2,x3) = 6,8+1,7*x1+5,7*x2+9,9*x3+8,9*x1*x1+0,5*x2*x2+6,1*x3*x3+9,9*x1*x2+0,7*x1*x3+3,2*x2*x3+8,4*x1*x2*x3')
print("-" * 100)
...
def main(m, n):
    ...
    if n == d:
        n = 8 if n == 4 else 14
        print(f"n=d\nПочнемо знову з n = {n} and m = {m}")
        return main(m=m, n=n)
    if n == 14:
        print(
            f"Нормалізоване спрощене рівяння регресії: "
    ...
    if n == 8:
        print(
            f"Нормалізоване рівняння регресії: "
    ...
    if f_p > get_value(f_t, f_3)[f_4]:
        ...
        print(
            ...
            f"Математична модель неадекватна з цими експериментальними даними\n"
            f"-----------------------------------------------------------------"
            f"\nПочнемо знову з m = {m} and n = {n}")
        return main(m=m, n=n)
    else:
        print(f"Тест тривав {(datetime.now() - start_time).total_seconds()} сек.")
        print(
